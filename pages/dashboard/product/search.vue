...
const handleConfirmDisable = async (item) => {
    customSwal.fire({
        ...
        if (result.isConfirmed) {
            await axios.get(`http://localhost:3050/api/v1/product/confirm/disable/${item.id}`)
                .then((res) => {
...
const handleConfirmEnable = async (item) => {
    customSwal.fire({
        ...
        if (result.isConfirmed) {
            await axios.get(`http://localhost:3050/api/v1/product/confirm/enable/${item.id}`)
                .then((res) => {
                    console.log(res.data.status)
...
const handleDelete = (item) => {
    customSwal.fire({
        ...
        if (result.isConfirmed) {
            await axios.get(`http://localhost:3050/api/v1/product/delete/${item.id}`)
                .then((res) => {
                    console.log(res.data.status)
    ...
    <section class="bg-white dark:bg-gray-900">
        ...
        <div class="overflow-x-auto">
            <table class="table table-xs">
                ...
                <tbody>
                    <tr v-for="(item, index) in products.filter(k => k.deletedAt === null)">
                        <th>{{ item.id }}</th>
                        <td>{{ item.title }}</td>
                        <td>{{ item.seo }}</td>
                        ...
                        <td class="inline-flex">
                            ...
                            <span v-if="item.confirm" class="cursor-pointer" @click="handleConfirmDisable(item)">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                                </svg>
                            </span>
                            <span v-if="!item.confirm" class="cursor-pointer" @click="handleConfirmEnable(item)">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
